...
use App\Features\Saves\Http\Controllers\Api\V1\SaveArtworkController;
use App\Features\Saves\Http\Controllers\Api\V1\SavedArtworkIndexController;
use App\Features\Saves\Http\Controllers\Api\V1\SavedArtworkStatusController;
use App\Features\Saves\Http\Controllers\Api\V1\StoreController;
use App\Features\Saves\Http\Controllers\Api\V1\UnsaveArtworkController;
...
Route::middleware(['auth:sanctum', 'account.active', 'role:maker,appreciator'])->group(function (): void {
    Route::get('/me/saved-artworks', SavedArtworkIndexController::class)->name('saved-artworks.index');
    Route::get('/me/saved-artworks/{artwork}', SavedArtworkStatusController::class)
        ->whereNumber('artwork')
        ->name('saved-artworks.status');
    Route::post('/me/saved-artworks/{artwork}', SaveArtworkController::class)
        ->whereNumber('artwork')
        ->name('saved-artworks.store');
    Route::delete('/me/saved-artworks/{artwork}', UnsaveArtworkController::class)
        ->whereNumber('artwork')
        ->name('saved-artworks.destroy');
});
